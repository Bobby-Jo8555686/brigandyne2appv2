...
export class BrigandyneActorSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  async _prepareContext(options) {
    ...
    context.armesEquipees = context.armes.filter(a => a.system.equipe);
    ...
    // ==========================================
    // PRÉPARATION DES COMPTEURS D'UTILISATION
    // ==========================================
    const allItems = [context.armes, context.armures, context.objets, context.atouts, context.sorts, context.tours, context.sortileges];
    for (let arr of allItems) {
        for (let i of arr) {
            if (i.system.activities) {
                for (let [actId, act] of Object.entries(i.system.activities)) {
                    if (act.utilisations?.max > 0) {
                        i.primaryActivityId = actId;
                        i.uses = { value: act.utilisations.value, max: act.utilisations.max };
                        break; // On prend la première activité avec un max > 0
                    }
                }
            }
        }
  ...
  _onRender(context, options) {
    ...
    html.find('[data-action="resetActivities"]').click(async ev => {
            ...
            new Dialog({
                ...
                content: `
                    ...
                    <form style="font-family: 'Georgia', serif; color: #333;">
                        <div style="margin-bottom: 8px;"><label style="cursor: pointer;"><input type="radio" name="restType" value="combat" checked> <b>Fin de combat</b> <span style="font-size: 0.85em; color: #777;">(Recharge "Par combat")</span></label></div>
                        <div style="margin-bottom: 8px;"><label style="cursor: pointer;"><input type="radio" name="restType" value="scene"> <b>Fin de Scène</b> <span style="font-size: 0.85em; color: #777;">(Recharge "Combat" et "Scène")</span></label></div>
                        <div style="margin-bottom: 15px;"><label style="cursor: pointer;"><input type="radio" name="restType" value="jour"> <b>Repos Long (Jour)</b> <span style="font-size: 0.85em; color: #777;">(Recharge tout sauf Permanent)</span></label></div>
                    </form>
                `,
                buttons: {
        ...
        if (confirm) {
            ...
            for (let item of this.actor.items) {
                ...
                for (let key in acts) {
                    ...
                    if (
                        ...
                        (confirm === "scene" && (freq === "combat" || freq === "session")) ||
                        (confirm === "jour" && (freq === "combat" || freq === "session" || freq === "jour"))
                    ) {
            ...
            } else { ui.notifications.info(`🏕️ ${this.actor.name} n'avait aucune capacité à recharger pour ce type de repos.`); }
        }
    });

    // ==========================================
    // CHANGEMENT MANUEL DU COMPTEUR SUR LA FICHE
    // ==========================================
    html.find('.use-input').change(async ev => {
        ev.preventDefault();
        const itemId = ev.currentTarget.dataset.itemId;
        const actId = ev.currentTarget.dataset.actId;
        let newVal = parseInt(ev.currentTarget.value) || 0;
        const item = this.actor.items.get(itemId);
        if (item && item.system.activities && item.system.activities[actId]) {
            let acts = foundry.utils.deepClone(item.system.activities);
            acts[actId].utilisations.value = newVal;
