export async function useActivity(itemId, activityId, forcedRU = null) {
        ...
        // 🔥 SÉCURITÉ V13 : On protège la lecture avec "?." au cas où les données soient incomplètes
        ...
        // 🔥 SÉCURITÉ V13 : Si l'effet n'existe pas, on met une chaîne vide pour éviter le crash du .toLowerCase()
    ...
    export async function _executeActivityFinal(item, act, finalFormula, ressource, coutVal) {
        ...
        // ==========================================
        // 3. GESTION DE LA SAUVEGARDE (Auto pour tous !)
        // ==========================================
        let effectApplies = true; 

        // 🔥 SECURITE V13 : On ajoute les "?." pour éviter le crash sur les vieux objets !
        if (act.effet?.sauvegarde && act.effet.sauvegarde.stat) {
            ...
            if (targetActor) {
                // Le jet se lance tout seul pour vérifier si les dégâts s'appliquent
                ...
                effectApplies = !isSuccess; // Si réussi, l'effet s'annule
        ...
        // 🔥 LES LIGNES SAUVEUSES SONT DE RETOUR ICI !
        htmlChat += `</div>`;
