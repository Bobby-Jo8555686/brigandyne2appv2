...
Hooks.once("init", async function() {
    ...
    game.settings.register("brigandyne2appv2", "advancedCombatEffects", {
        name: "Effets Martiaux Avancés",
        hint: "Affiche la liste complète des effets spéciaux (Choc, Désarmement, Précision, etc.) dans le menu des réussites majeures. Si décoché, seuls les effets de base et le Coup Dévastateur seront proposés, pour des combats plus rapides.",
        scope: "world",
        config: true,
        type: Boolean,
        default: false 
    ...
    Hooks.once("ready", () => {
        document.body.addEventListener("click", async (ev) => {
            ...
            if (btn) {
                ...
                let boutonsEtatsHtml = "";

                const addStatusButton = (label, icon, statusKey) => {
                    boutonsEtatsHtml += `
                    <button class="apply-status-btn" data-actor-id="${loserActor ? loserActor.id : ''}" data-status="${statusKey}" style="width: 100%; margin-top: 4px; background: #b71c1c; color: white; border: none; padding: 5px; border-radius: 3px; cursor: pointer; font-weight: bold;">
                        ${icon} Appliquer l'état ${label}
                    </button>`;
                };
                ...
                const checkEffect = (eff) => {
                    switch(eff) {
                        ...
                        case "enchainement": 
                            appliedEffectsText.push("⚡ <b>Enchaînement</b> (Nouvelle attaque débloquée)"); 
                            let armeEnchainement = winnerActor.items.find(i => i.type === "arme" && i.system.equipe && i.system.type_arme !== "distance");
                            if (armeEnchainement) {
                                boutonsEtatsHtml += `
                                <div style="text-align: center; margin-top: 5px;">
                                    <button class="enchainement-btn" data-actor-id="${winnerActor.id}" data-weapon-id="${armeEnchainement.id}" style="background: #e65100; color: white; border: 1px solid #e65100; padding: 4px; border-radius: 3px; cursor: pointer; font-weight: bold; width: 100%;">
                                        ⚔️ Lancer l'Enchaînement (Dégâts/2, Sans Riposte)
                                    </button>
                                </div>`;
                            }
                            break;
                        ...
                        // LES EFFETS À BOUTON (Activités Fictives)
                        case "choc": 
                            appliedEffectsText.push("💫 <b>Choc</b> (Cible Sonnée)"); 
                            addStatusButton("Sonné", "💫", "sonne"); 
                            break;
                        case "handicapant": 
                            appliedEffectsText.push("🦵 <b>Coup handicapant</b> (Cible Ralentie)"); 
                            addStatusButton("Ralenti", "🦵", "ralenti"); 
                            break;
                        case "illegal": 
                            appliedEffectsText.push("👁️ <b>Coup illégal</b> (Cible Affaiblie)"); 
                            addStatusButton("Affaibli", "👁️", "affaibli"); 
                            break;
                        case "saignement": 
                            appliedEffectsText.push("🩸 <b>Saignement</b> (Cible Ensanglantée)"); 
                            addStatusButton("Ensanglanté", "🩸", "ensanglante"); 
                            break;
                    }
                };
                ...
                if (isBlesser && !payload.degatsNul) {
                    ...
                    if (loserActor) {
                        ...
                        let isSbireKilled = false;
                        if (payload.sbireOneHit && loserActor.type === "pnj" && loserActor.system.type_pnj === "sbire" && finalDamage > 0) {
                            newHp = 0;
                            isSbireKilled = true;
                        }
                        ...
                        <div class="weapon-damage ${boxColor}" style="margin-top: 10px; padding: 10px; border-radius: 5px; background: rgba(0,0,0,0.1); border: 1px solid rgba(255,255,255,0.2);">
                            ...
                            ${isSbireKilled ? `<div style="margin-top: 8px; color: #fff; background: rgba(183, 28, 28, 0.8); padding: 5px; text-align: center; border-radius: 3px; font-weight: bold; text-shadow: 1px 1px 2px black;">💥 Le sbire est vaincu d'un coup !</div>` : ''}
                        </div>`;
                    }
                } else {
            ...
            if (tieBtn) {
                ...
                if (tieType === "parade") {
                    ...
                    if (pnjActor) {
                        ...
                        if (payload.sbireOneHit && pnjActor.type === "pnj" && pnjActor.system.type_pnj === "sbire" && dmgToPnj > 0) {
                            newPnjHp = 0;
                            sbireText = `<div style="color: #ffaaaa; font-weight: bold; margin-top: 4px; font-size: 0.9em; border-top: 1px dashed #b71c1c; padding-top: 3px;">💥 Sbire éliminé !</div>`;
                        }
            ...
            // ==========================================
            // GESTION DES BOUTONS D'ÉTATS (Activités Fictives)
            // ==========================================
            btn = ev.target.closest(".apply-status-btn");
            if (btn) {
                ev.preventDefault();
                ...
                // Sécurité : Seul le MJ ou le proprio du token peut appliquer l'état
                if (!targetActor.isOwner) return ui.notifications.warn("Seul le MJ peut appliquer cet état au PNJ !");

                // Mise à jour de la fiche du PNJ
                let updateData = {};
                updateData[`system.handicaps.${statusKey}`] = true;
                await targetActor.update(updateData);

                // On désactive le bouton pour éviter les doubles clics
                btn.disabled = true;
                btn.innerHTML = `✅ État appliqué !`;
                btn.style.background = "#4CAF50";
                return;
            }
            // ==========================================
            // 8. GESTION DE L'ENCHAÎNEMENT
            // ==========================================
            let enchBtn = ev.target.closest(".enchainement-btn");
            if (enchBtn) {
                ev.preventDefault();
                const actor = game.actors.get(enchBtn.dataset.actorId);
                if (!actor || !actor.isOwner) return ui.notifications.warn("Vous ne possédez pas ce personnage !");
                
                enchBtn.disabled = true;
                enchBtn.innerHTML = `<i class="fas fa-check"></i> Enchaînement lancé !`;
                enchBtn.style.opacity = "0.5";

                try {
                    // On lance l'arme en lui glissant nos options secrètes
                    await actor.rollWeapon(enchBtn.dataset.weaponId, { forcedSansRiposte: true, isEnchainement: true });
                } catch (err) { console.error(err); }
                return;
            }
        });
