export async function rollSpell(itemId) {
    ...
    export async function _executeSpellRoll(sort, options, forcedResult = null) {
        ...
        if (isSuccess && (forcedResult === null || forcedResult === false)) {
            ...
            if (actKeys.length === 1) {
                // 🔥 On passe la cible !
                await this.useActivity(sort.id, actKeys[0], finalRu, options.targetId);
            } else if (actKeys.length > 1) {
                let buttons = {};
                for (let k of actKeys) {
                    // 🔥 On passe la cible ici aussi !
                    buttons[k] = { icon: '<i class="fas fa-bolt"></i>', label: acts[k].nom, callback: () => this.useActivity(sort.id, k, finalRu, options.targetId) };
                }
                new Dialog({
