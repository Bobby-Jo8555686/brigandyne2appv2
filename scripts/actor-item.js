export async function rollItem(itemId) {
    ...
    <form>
        ...
        <div class="form-group" style="margin-bottom: 15px; padding: 10px; background: rgba(0,0,0,0.8); border-radius: 5px; border: 1px solid #d4af37;">
            <label style="font-weight: bold; color: #d4af37;">Bonus / Malus libre (+/-) :</label>
            <input type="number" id="bonusMalusLibre" value="0" step="1" style="width: 100%; text-align: center; background: rgba(0,0,0,0.5); color: #fff; border: 1px solid #555; margin-top: 5px;">
        </div>
    </form>
    `;
        ...
        buttons: {
            roll: {
                ...
                callback: async (html) => {
                    ...
                    // NOUVEAU : Récupération de la valeur
                    const bonusMalusLibre = parseInt(html.find('#bonusMalusLibre').val()) || 0;
                    
                    // NOUVEAU : Passage en paramètre (à la fin)
                    await this._executeItemRoll(item, statKey, modDifficulte, advC, bonusMalusLibre);
                }
            },
            useDirect: {
...
// NOUVEAU : Ajout de bonusMalusLibre = 0 dans la signature
export async function _executeItemRoll(item, statKey, modDifficulte, advC, bonusMalusLibre = 0) {
    const stat = this.system.stats[statKey];
    let scoreBase = stat.total !== undefined ? stat.total : stat.value;
    
    // NOUVEAU : On l'ajoute au score final
    let score = scoreBase + modDifficulte + advC + bonusMalusLibre;
    ...
    <div style="font-size: 0.9em; background: rgba(0,0,0,0.4); padding: 5px; border-radius: 3px; margin-bottom: 8px; text-align: left; border: 1px solid #444;">
        ...
        ${bonusMalusLibre !== 0 ? `<div><strong style="color:#e0e0e0;">Bonus libre :</strong> <span style="color:#d4af37;">${bonusMalusLibre > 0 ? '+'+bonusMalusLibre : bonusMalusLibre}</span></div>` : ''}
