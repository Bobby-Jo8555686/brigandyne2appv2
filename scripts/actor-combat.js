export async function rollWeapon(itemId) {
    ...
    export async function _executeWeaponRoll(weapon, options, forcedResult = null) {
        ...
        // GÉNÉRATION DU BOUTON (Arme Fétiche / Doué)
        if (forcedResult === null || forcedResult === false) {
            const hasArmeFetiche = hasTalent("arme fetiche");
        ...
        if (isSuccess && (forcedResult === null || forcedResult === false)) {
            ...
            if (actKeys.length === 1) { this.useActivity(weapon.id, actKeys[0], ru); } 
            else if (actKeys.length > 1) {
                let buttons = {}; for (let k of actKeys) { buttons[k] = { icon: '<i class="fas fa-bolt"></i>', label: acts[k].nom, callback: () => this.useActivity(weapon.id, k, ru) }; }
